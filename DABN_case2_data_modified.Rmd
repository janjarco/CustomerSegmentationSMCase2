...
#Read it
click_order_merge_raw <- read_csv2("click_order_merged.csv", col_types = cols(.default = "?", order_user_ipaddress = col_character()))
```
...
click_order_merge <- click_order_merge_raw %>% 
  select(-c(
    # click_columns
...
orders %>% select(timestamp_date, paid_at_date, paid, cancelled_at, cancelled) %>% 
  filter(cancelled == T) %>%
  mutate(
    cancelled_at = as.POSIXct(strptime(cancelled_at, format = "%Y-%m-%dT%H:%M:%S")), 
    timestamp_date = as.POSIXct(strptime(timestamp_date, format = "%Y-%m-%dT%H:%M:%S")),
    paid_at_date = as.POSIXct(strptime(paid_at_date, format = "%Y-%m-%dT%H:%M:%S")), 
    timestamp_cancelled_timeperiod = cancelled_at - timestamp_date
    ) %>% .$timestamp_cancelled_timeperiod %>% table

click_order_merge %>% select(order_if_order, order_search_data.gate_name) %>% table %>% t
  
```
```{r}

click_order_merge %>% 
  select(click_itinerary_string, order_if_order) %>% 
  table %>% as.data.frame() %>% 
  pivot_wider(id_cols = "click_itinerary_string", names_from = "order_if_order", names_prefix = "if_order_", values_from = "Freq") %>% mutate(total = if_order_0 + if_order_1, conversion_rate = if_order_1 / total) %>% arrange(desc(total)) %>% 
  filter(total > 100) %>% 
  ggplot(., aes(x = total, y = conversion_rate)) +
  geom_point() +
  ggtitle("Conversion rates for certain routes") + 
  xlab("Total number of clicks") +
...
click_order_merge %>% 
  select(click_mobile, order_if_order) %>% 
  table %>% as.data.frame() %>% mutate(click_mobile = as.factor(if_else(click_mobile == TRUE, "mobile", "desktop"))) %>% 
  pivot_wider(id_cols = "click_mobile", names_from = "order_if_order", names_prefix = "if_order_", values_from = "Freq") %>% mutate(total = if_order_0 + if_order_1, conversion_rate = if_order_1 / total) %>% arrange(desc(total)) %>% 
  filter(total > 100) %>% 
  ggplot(., aes(x = total, y = conversion_rate)) +
  geom_point() #+
  scale_x_continuous(trans='log')
...
```{r}
click_order_merge %>% 
  mutate(click_result_set_price_chosen = as.factor(dplyr::case_when(
  click_itinerary_sales_price == click_result_set_cheapest_price ~ "cheapest",
  click_itinerary_sales_price == click_result_set_best_price ~ "best",
  click_itinerary_sales_price == click_result_set_fastest_price ~ "fastest", 
  TRUE ~ NA_character_))) %>% 
  select(click_result_set_price_chosen, order_if_order) %>% 
  table %>% as.data.frame() %>% 
  pivot_wider(id_cols = "click_result_set_price_chosen", names_from = "order_if_order", names_prefix = "if_order_", values_from = "Freq") %>% mutate(total = if_order_0 + if_order_1, conversion_rate = if_order_1 / total) %>% arrange(desc(total))
  
```

click_itinerary_direct_flight
```{r}

click_order_merge %>% 
  select(click_itinerary_direct_flight, order_if_order) %>% 
  table %>% as.data.frame() %>% #mutate(click_mobile = as.factor(if_else(click_mobile == TRUE, "mobile", "desktop"))) %>% 
  pivot_wider(id_cols = "click_itinerary_direct_flight", names_from = "order_if_order", names_prefix = "if_order_", values_from = "Freq") %>% mutate(total = if_order_0 + if_order_1, conversion_rate = if_order_1 / total) %>% arrange(desc(total)) %>% 
  # filter(total > 100) %>% 
  ggplot(., aes(x = total, y = conversion_rate)) +
  geom_point() #+
  scale_x_continuous(trans='log')
...
```{r} 
click_order_merge %>% 
  select(order_if_order, click_itinerary_travel_time) %>% 
  mutate(order_if_order = as.factor(order_if_order)) %>% 
  ggplot( aes(x=click_itinerary_travel_time, fill=order_if_order)) +
    geom_histogram( color="#e9ecef", alpha=0.6, position = 'fill', binwidth = 100) +
    scale_fill_manual(values=c("#69b3a2", "#404080")) +
    xlim(0, 4000)
    # theme_ipsum() +
    labs(fill="") 
...
get_location <- function(ip_address) {
  # browser()
  url <- paste0('https://ipapi.co/', ip_address, '/json/')
  response <- GET(url)
  ...
  location_data <- list(
    "ip" = ip_address,
    "city" = response_list$city,
    "region" = response_list$region,
    "country" = response_list$country_name
  )
  # print(paste(ip_address, " extracted"))
  return(location_data)
}

get_location(click_order_merge$order_user_ipaddress[1])

click_order_merge_filtered <- click_order_merge %>% filter(order_if_order == T) 

suppressWarnings()
temp <- suppressWarnings({sapply(c(1:nrow(click_order_merge_filtered)), function(i){
  print(paste(i / nrow(click_order_merge_filtered), "made"))
  return(get_location(click_order_merge_filtered$order_user_ipaddress[i]) )
}, simplify = F, USE.NAMES = F) })

ip_adresses_location <- sapply(temp , unlist) %>% do.call("rbind", .) %>% as.data.frame() 
write.csv2(ip_adresses_location, "ip_adresses_location.csv", row.names = F)
```

Adding airports distances
